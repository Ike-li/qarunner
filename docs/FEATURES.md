...
一个**安全隔离的「外部测试代码」执行与调度平台**:把第三方 / 团队的 pytest、playwright 测试代码接进来,在受控沙箱中运行,收集结果、生成 Allure 报告、支持 cron 定时,全程配 Web 控制台与 REST API。

核心取舍:把「运行不受信任的测试代码」这件高风险的事做**重**(多层隔离),把外围功能(通知 / 导出 / 多租户)**留白**,换取部署简单、易于审计。
