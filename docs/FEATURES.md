...
一个**自托管、单节点的测试执行 + 调度 + 回归对比服务**:把团队的 pytest、playwright 测试套件接进来,定时或即席运行(每次都跑在**硬化的一次性沙箱**里),收集结果、生成 Allure 报告;并把历次运行连成**回归视图**——基线 diff、通过率趋势、flaky 检测、用例级历史。全程配 Web 控制台与 REST API。

**定位边界**(完整推理见 [`POSITIONING.md`](POSITIONING.md)):它是 test **runner** + scheduler + reporter **+ 回归视图**(跨次对比是它区别于「跑一次看一次」runner 的差异化内核)——**不是** CI(无 VCS 集成 / 流水线 / 多阶段构建)、**不是** QA 平台(无用例 / 缺陷管理 / 需求追溯)、**不是**多租户 SaaS(仅用户级 owner-scope)。
